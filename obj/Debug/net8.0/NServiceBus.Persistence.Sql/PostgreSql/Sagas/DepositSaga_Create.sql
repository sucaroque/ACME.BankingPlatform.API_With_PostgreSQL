...
create or replace function pg_temp.create_saga_table_DepositSaga(tablePrefix varchar, schema varchar)
    returns integer as
    $body$
    declare
...
/* WriteCreateIndex TransactionId */
        ...
        script = 'create unique index if not exists "' || tablePrefix || '_i_7D5D32E094A304AF96132135A31416154DB93037" on "' || schema || '"."' || tableNameNonQuoted || '" using btree ("Correlation_TransactionId" asc);';
        execute script;
/* PurgeObsoleteIndex */
...
select pg_temp.create_saga_table_DepositSaga(@tablePrefix, @schema);
