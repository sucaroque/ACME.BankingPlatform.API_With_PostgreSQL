...
create or replace function pg_temp.create_saga_table_TransferSaga(tablePrefix varchar, schema varchar)
    returns integer as
    $body$
    declare
...
/* WriteCreateIndex TransactionId */
        ...
        script = 'create unique index if not exists "' || tablePrefix || '_i_4F3E43575584FB700E723CBBC61023AB106CC2EE" on "' || schema || '"."' || tableNameNonQuoted || '" using btree ("Correlation_TransactionId" asc);';
        execute script;
/* PurgeObsoleteIndex */
...
select pg_temp.create_saga_table_TransferSaga(@tablePrefix, @schema);
